...
import { ChevronLeft, ChevronRight, Search, Building2, Users, TrendingUp, TrendingDown, PiggyBank, HandCoins, Eye } from 'lucide-react';
...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [list, stats] = await Promise.all([
        supabase.rpc('admin_tenants_overview_paginated', {
          _limit: PAGE_SIZE, _offset: page * PAGE_SIZE, _search: debouncedSearch || null,
        }),
  ...
  }, [isSuperAdmin, page, debouncedSearch]);
              ...
              <table className="w-full text-sm">
                <thead className="text-left text-xs text-muted-foreground uppercase tracking-wider">
                  <tr className="border-b">
                    ...
                    <th className="py-2 pl-2">Last Activity</th>
                  ...
                  {rows.map(r => (
                    <tr key={r.tenant_id} className="border-b hover:bg-muted/30">
                      ...
                      <td className="py-2 px-2 text-center">
                        <Badge variant="outline">{r.current_users}/{r.max_users}</Badge>
                      </td>
                      ...
                      <td className="py-2 pl-2 text-xs text-muted-foreground">
                        {r.last_activity ? format(new Date(r.last_activity), 'MMM d') : '—'}
                      </td>
           ...
           drilldown && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                ...
                <MiniStat label="Users" value={fmt(drilldown.users?.length ?? 0)} />
              </div>
              ...
              {drilldown.category_breakdown?.length > 0 && (
                <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Top categories</CardTitle></CardHeader>
                <CardContent>
                  <div className="space-y-1">
                    {drilldown.category_breakdown.map((c: any, i: number) => (
                      <div key={i} className="flex justify-between text-sm py-1 border-b">
                        <span><Badge variant="outline" className="mr-2 text-[10px]">{c.type}</Badge>{c.category}</span>
                        <span className={c.type === 'INCOME' ? 'text-income font-medium' : 'text-expense font-medium'}>{fmt(c.amount)} RWF</span>
                      </div>
                    ))}
                  </div>
                </CardContent></Card>
              ...
              {drilldown.users?.length > 0 && (
                <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Users</CardTitle></CardHeader>
                <CardContent>
                  <div className="space-y-1 text-sm">
                    {drilldown.users.map((u: any) => (
                      <div key={u.user_id} className="flex justify-between py-1 border-b">
